...
let stringLiteral: DexterParser<_> =
    let stringContent (closingQuote: char) = 
        ...
        let normalCharacter (closingQuote: char) = satisfy (fun x -> x <> '\\' && x <> closingQuote) 
        many (normalCharacter closingQuote <|> escapedCharacter closingQuote) .>> skipChar closingQuote |>> (List.map string >> String.concat "")

    (skipChar '\'' >>. stringContent '\'') <|> (skipChar '"' >>. stringContent '"') |>> StringLiteral
    
let numberFormat = NumberLiteralOptions.AllowBinary |||
