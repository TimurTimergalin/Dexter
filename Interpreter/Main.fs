            ...
            | Operator(name', opt) ->
                let nonOptionalOperator =
                    if ns <> [] then
                        ...
                        | Ok op -> op
                        | Error _ ->
                            Value.Function (fun st v -> Value.Application(defaultOperator st name' v, v))

                if not opt then nonOptionalOperator else optionalOperator nonOptionalOperator
    ...
    | Ref(ctx, getter, setter) ->
        ...
        | Object(cons, args) ->
            ...
            Object(
                cons,
                List(args)
                |> mapi (fun i _ -> Ref(ctx, newGetter i, newSetter i))
            )
